...
const navItems = [
    ...
    { href: "/student/materials", label: "Course Materials", icon: "lucide:book-open" },
    { href: "/student/forum", label: "Forum", icon: "lucide:message-square" },
]

const aiTools = [
    { href: "/student/chat", label: "Chat Assistant", icon: "lucide:bot" },
    { href: "/student/generator", label: "Content Generator", icon: "lucide:sparkles" },
    { href: "/student/digitize", label: "Digitize Notes", icon: "lucide:scan-line" },
]
...
function StudentShell({ children }: { children: React.ReactNode }) {
    ...
    const displayName = profile?.displayName || profile?.email || "Student"
    ...
    const NavLink = ({ href, label, icon }: { href: string; label: string; icon: string }) => {
        ...
        return (
            <Link
                ...
                className={`group flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition-all duration-200 ${isActive
                    ? "bg-primary text-primary-foreground shadow-warm-sm"
                    : "text-muted-foreground hover:bg-muted hover:text-foreground"
                    }`}
            >
                <Icon icon={icon} className={`h-5 w-5 ${isActive ? "" : "group-hover:scale-110"} transition-transform`} />
                {label}
            </Link>
        )
    }
            ...
            <div className="flex h-20 items-center px-6">
                <Link href="/student/dashboard" className="flex items-center gap-2">
                    <div className="h-9 w-9 rounded-xl bg-primary flex items-center justify-center">
                        <Icon icon="lucide:graduation-cap" className="h-5 w-5 text-primary-foreground" />
                    </div>
            ...
            <div className="flex-1 overflow-y-auto py-4 px-4">
                <nav className="space-y-2">
                    ...
                    <div className="pt-6 pb-2">
                        <p className="px-4 text-xs font-semibold text-muted-foreground/70 uppercase tracking-wider">
                            AI Tools
                        </p>
                    </div>
                    {aiTools.map((item) => (
    ...
    return (
        <div className="flex min-h-screen bg-background">
            <aside className="hidden md:flex w-72 flex-col border-r border-border/50 bg-card/50 backdrop-blur-sm fixed h-full z-30">
                <SidebarContent />
            </aside>
